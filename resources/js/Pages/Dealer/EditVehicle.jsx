...
import { Car, Plus, ArrowLeft, Building2, Upload, Trash2, Image, CheckCircle2, Sparkles, Check, X, ShieldCheck, Save } from 'lucide-react';
...
export default function EditVehicle({ vehicle }) {
  ...
  const [images, setImages] = useState(vehicle.images_json || [
    'https://images.unsplash.com/photo-1614162692292-7ac56d7f7f1e?auto=format&fit=crop&w=1000&q=80',
  ]);

  const [features, setFeatures] = useState(vehicle.features_json || [
    '150-Point TÜV / DEKRA Certificate',
    'Adaptive Air Suspension',
  ]);
  ...
  const { data, setData, post, processing, errors } = useForm({
    ...
    color: vehicle.color || 'Factory Finish',
    location_country: vehicle.location_country || 'CH',
    location_city: vehicle.location_city || 'Zurich',
    status: vehicle.status || 'available',
    is_fleet_eligible: vehicle.is_fleet_eligible || false,
    images: images,
    features: features,
  });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    post(route('dealer.vehicles.update', vehicle.id));
  };
  ...
  return (
    ...
    >
      <Head title={`Edit ${vehicle.make} ${vehicle.model} - Dealer Portal`} />
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-8 sm:p-10 shadow-xl space-y-8">
          ...
          <div className="flex items-center justify-between pb-6 border-b border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-black text-slate-900">Edit Vehicle Specification & Pricing</h1>
                <p className="text-xs text-slate-500 font-medium mt-0.5 font-mono">
                  VIN: {vehicle.vin} &bull; ID: #{vehicle.id}
                </p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8 text-xs font-semibold">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="available">Available (Active Sourcing)</option>
                    <option value="reserved">Reserved (Deposit Holding)</option>
            ...
            >
              ...
              <span>Update Vehicle Listing Details</span>
            </button>
          </form>
        </div>
      </div>
